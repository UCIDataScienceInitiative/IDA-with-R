...
### Part A - Data Input
**A.1** Open a new R script file to write and save your code for the exercises. Set your working directory to your local copy of the IDA-with-R GitHub repo using the ```setwd()``` function.
```{r echo=TRUE}
...
**D.5** Save your data set as an R data (```.Rda```) file in the data directory (i.e.,  ```"/data/auto_mpg_v2.Rda"```) using the ```save()``` function.
```{r echo=TRUE}
